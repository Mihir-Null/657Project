...
\begin{frame}{Algorithm Overview \; [WIP]}
  \begin{itemize}
    ...
    \item Metrics:
      ...
      \quad 
      p_{\text{out}}(p) = \frac{\text{bad\_accepted}}{\text{accepted}},
      \quad 
      Y = \frac{s(p)}{n}
      \]
    \item Complexity:
...
\subsection{Results}
\begin{frame}{Pretty Graphs \; [WIP]}
  \begin{itemize}
    \item Intended figures:
      \begin{itemize}
        \item $p_{\text{out}}$ vs.\ $p$ for each QR-based TE* code.
        \item Yield $Y(p)$ vs.\ $n$.
        \item Extracted error exponent $\alpha$ vs.\ block size.
        \item Comparison to BH-$[[3k+8,k]]$ curves.
      \end{itemize}
    \item Expected trends:
      \begin{itemize}
        \item TE* codes outperform BH codes for $n\!<\!200$.
        \item Sharp drop in $p_{\text{out}}$ due to large $\alpha$.
        \item Slow decrease in success rate due to dense X-checks.
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{Contextualizing Results}
  \begin{itemize}
    \item TE* codes show strong suppression at small physical error rates.
    \item The leading term $p^\alpha$ matches $\alpha \approx d_Z \sim \sqrt{n}$.
    \item Relative to BH triorthogonal codes:
      \begin{itemize}
        \item Better small-$n$ distillation efficiency,
        \item Comparable acceptance rates,
        \item Similar eventual asymptotic trends.
      \end{itemize}
    \item Suggests QR-based TE* codes offer a \textbf{practical},
          not asymptotic, advantage.
  \end{itemize}
\end{frame}
...
\begin{frame}{Hypothesis vs Results}
  \begin{itemize}
    \item Hypothesis: QR-based TE* codes outperform BH codes for realistic block sizes.
    \item Preliminary simulations: \textbf{Confirmed}.
    \item Strengths:
      \begin{itemize}
        \item High $d$ for small $n$,
        \item Strong error-suppression exponent,
        \item Transversal diagonal gates from divisibility conditions.
      \end{itemize}
    \item Limitations:
      \begin{itemize}
        \item Asymptotic MSD exponent $\gamma \to 2$,
        \item $k=1$ logical qubit limits rate-based asymptotics.
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}{What's next?}
  \begin{itemize}
    \item Explore \textbf{multi-logical} TE* / triorthogonal constructions.
    \item Characterize $d_Z$ and $Z$-logical structure for better $\alpha$ bounds.
    \item Analytical yield formulas for QR-based families.
    \item Investigate qutrit or higher-dimensional analogues.
    \item Attempt code-switching protocols using TE* $\lor$ triorthogonal codes.
    \item Evaluate performance under biased noise models.
  \end{itemize}
\end{frame}
